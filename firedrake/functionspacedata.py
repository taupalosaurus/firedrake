...
def get_boundary_masks(mesh, key, finat_element):
    ...
    chart = sum(map(len, itervalues(ecd))) - 1
    ...
    facet_points = []
    p = 0
    for ent in sorted(iterkeys(ecd)):
        # Never need closure of cell
        if sum(ent) == dim:
            continue
        for key in sorted(iterkeys(ecd[ent])):
    ...
    def get_map(self, V, entity_set, map_arity, bcs, name, offset, parent):
        ...
        try:
            ...
            if self.boundary_masks is None:
                ...
                for name, (section, iset, facet_iset) in iteritems(self.boundary_masks):
